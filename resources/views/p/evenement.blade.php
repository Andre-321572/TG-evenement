...
@section('content')
    ...
    <div class="container py-5 text-slate-800">
        <div class="mb-4">
            <h1 class="fw-bold text-slate-900 fs-3 mb-4" style="font-family: 'Outfit', sans-serif;">Trouver un événement
            </h1>
        </div>
        ...
        <form method="GET" action="{{ route('p.evenement') }}" id="filter-form">
                ...
                <div class="row g-3 align-items-center">
                    ...
                    <div class="col-md-5 d-flex align-items-center px-4 border-end border-slate-100 last:border-end-0">
                        ...
                        <input type="text" name="search" id="search-input" value="{{ request('search') }}"
                            placeholder="Rechercher des concerts, conférences, sports..."
                            class="form-control bg-transparent border-0 shadow-none text-slate-800 text-sm py-1.5 focus:ring-0 focus:outline-none w-100">
                    </div>
                    <!-- Location -->
                    <div class="col-md-4 d-flex align-items-center px-4">
                        <i class="fas fa-map-marker-alt text-slate-400 me-2.5 fs-5"></i>
                        <input type="text" name="lieu" id="lieu-input" value="{{ request('lieu') }}"
                    ...
                    <div class="col-md-3">
                        <button type="submit"
                            class="btn w-100 py-2.5 text-white font-semibold rounded-xl border-0 shadow-sm transition-all duration-200"
                            style="background: #1c2434; hover: background: #121824;">
                            Rechercher
                        </button>
                    </div>
                </div>
            ...
            <div class="row g-4 mt-2">
                ...
                <div class="col-lg-3 col-md-4">
                    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 sticky-top"
                        style="top: 96px; z-index: 10;">

                        <!-- Categories filter -->
                        <div class="mb-4">
                            <h6 class="text-slate-500 font-bold uppercase tracking-wider mb-3"
                                style="font-size: 0.75rem; letter-spacing: 0.05em;">Catégories</h6>
                            <div class="d-flex flex-column gap-2.5">
                                @foreach($categories as $cat)
                                    ...
                                    @endphp
                                    <label
                                        class="d-flex align-items-center gap-2.5 cursor-pointer text-slate-700 text-sm hover:text-indigo-600 mb-0 font-medium">
                        ...
                        <div class="mb-4">
                            <h6 class="text-slate-500 font-bold uppercase tracking-wider mb-3"
                                style="font-size: 0.75rem; letter-spacing: 0.05em;">Prix</h6>
                            @php
                                $currentPrixMax = request('prix_max', 350);
                            @endphp
                            <div class="px-1">
                                <input type="range" name="prix_max" id="prix_max_range" min="0" max="500" step="10"
                                    value="{{ $currentPrixMax }}" class="range-slider">
                                <div class="d-flex justify-content-between text-slate-500 font-semibold mt-2"
                        ...
                        <div class="mb-4">
                            <h6 class="text-slate-500 font-bold uppercase tracking-wider mb-3"
                                style="font-size: 0.75rem; letter-spacing: 0.05em;">Date</h6>
                            <div class="d-flex gap-2">
                                <button type="button" id="btn-weekend" onclick="setDateRange('weekend')"
                                    class="btn btn-sm flex-fill py-2 px-3 rounded-xl border border-slate-200 text-slate-700 font-semibold bg-slate-50 hover:bg-slate-100 text-xs transition-colors">
                                    Ce week-end
                                </button>
                                <button type="button" id="btn-month" onclick="setDateRange('month')"
                                    class="btn btn-sm flex-fill py-2 px-3 rounded-xl border border-slate-200 text-slate-700 font-semibold bg-slate-50 hover:bg-slate-100 text-xs transition-colors">
                                    Ce mois
                                </button>
                            </div>
                        ...
                        <div class="pt-2">
                            <a href="{{ route('p.evenement') }}"
                                class="btn w-100 py-2 border border-[#d9383a] text-[#d9383a] bg-white hover:bg-red-50 font-bold rounded-xl text-sm transition-colors text-decoration-none d-flex justify-content-center align-items-center border-2">
                                Réinitialiser
                            </a>
                        </div>
                    </div>
                ...
                <div class="col-lg-9 col-md-8">
                    <div class="d-flex justify-content-between align-items-center mb-4 pb-2">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <select name="sort" id="sort-select"
                                class="form-select bg-transparent border-0 text-slate-800 font-bold text-sm py-1 pe-8 focus:ring-0 cursor-pointer shadow-none">
                                <option value="priority" {{ request('sort') == 'priority' || !request('sort') ? 'selected' : '' }}>Pertinence</option>
                                <option value="date_asc" {{ request('sort') == 'date_asc' ? 'selected' : '' }}>Le plus proche
                                </option>
                                <option value="date_desc" {{ request('sort') == 'date_desc' ? 'selected' : '' }}>Le plus
                                    lointain</option>
                                <option value="titre_asc" {{ request('sort') == 'titre_asc' ? 'selected' : '' }}>Titre (A-Z)
                                </option>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="row g-4" id="events-grid-container">
                            @foreach($events as $event)
                                ...
                                <div class="col-md-6 col-lg-4">
                                    <div
                                        class="bg-white rounded-2xl border border-slate-100 hover:shadow-lg transition-all duration-300 h-100 flex flex-col justify-between overflow-hidden shadow-sm">
                                        <!-- Image Container -->
                                        <div class="position-relative overflow-hidden" style="height: 180px;">
                                            <img src="{{ $event->photo_url }}" alt="{{ $event->titre }}"
                                                class="w-100 h-100 object-cover" style="transition: transform 0.5s ease;">

                                            <!-- Date Badge -->
                                            <div class="position-absolute top-3 start-3 bg-white rounded-xl shadow-md d-flex flex-column align-items-center justify-content-center p-2"
                                                style="width: 46px; height: 50px; line-height: 1.1;">
                                                <span
                                                    class="fw-bold text-slate-900 fs-5 mb-0">{{ \Carbon\Carbon::parse($event->date)->format('d') }}</span>
                                                <span class="text-[#d9383a] font-extrabold uppercase"
                                            ...
                                            <!-- Heart Icon Button -->
                                            <div
                                                class="position-absolute top-3 end-3 w-9 h-9 rounded-full bg-black/40 backdrop-blur-xs d-flex align-items-center justify-content-center text-white cursor-pointer btn-heart transition-all duration-200">
                                        ...
                                        <div class="p-3 flex-grow-1 flex flex-col justify-between">
                                            <div>
                                                ...
                                                <div class="d-flex align-items-center gap-2 mb-2 text-xs">
                                                    <span
                                                        class="text-indigo-600 font-bold uppercase tracking-wider">{{ $event->categorie ?? 'Événement' }}</span>
                                                ...
                                                <!-- Title -->
                                                <h5 class="fw-bold text-slate-900 mb-2 fs-6 line-clamp-1"
                                                    style="font-family: 'Outfit', sans-serif;">
                                                    <a href="{{ route('p.detail', $event->id) }}"
                                                        class="text-slate-900 text-decoration-none hover:text-indigo-600">{{ $event->titre }}</a>
                                                </h5>

                                                <!-- Description -->
                                                <p class="text-slate-500 text-xs mb-3 line-clamp-2 leading-relaxed">
                                                    {{ $event->truncated_description }}</p>
                                            </div>

                                            <!-- Location & Price Row -->
                                            <div
                                                class="d-flex justify-content-between align-items-center border-t border-slate-100 pt-3 mt-auto">
                    ...
                    @else
                        <div class="text-center py-5 bg-white border border-slate-100 rounded-2xl shadow-sm">
                            ...
                            <p class="text-slate-400 small">Modifiez vos filtres de recherche ou réinitialisez pour afficher
                                tous les événements.</p>
                            <a href="{{ route('p.evenement') }}"
                                class="btn px-4 py-2 mt-2 rounded-xl text-white font-semibold border-0"
